...
const AddAndRemoveMarkers: React.FC = () => {
  ...
  const onMarkerClick = (data: any) => {
    setCommandOutput(
      `MARKER (${data.markerId}) WAS CLICKED ON MAP (${data.mapId})`,
    );
  };

  const onMarkerDragStart = (data: any) => {
    setCommandOutput(
      `MARKER (${data.markerId}) DRAG STARTED ON MAP (${data.mapId})`,
    );
  };

  const onMarkerDrag = (data: any) => {
    setCommandOutput(
      `MARKER (${data.markerId}) IS BEING DRAGGED ON MAP (${data.mapId})`,
    );
  };

  const onMarkerDragEnd = (data: any) => {
    setCommandOutput(
      `MARKER (${data.markerId}) DRAG ENDED ON MAP (${data.mapId})`,
    );
  };

  const onInfoWindowClick = (data: any) => {
    setCommandOutput(
      `INFO WINDOW (${data.markerId}) WAS CLICKED ON MAP (${data.mapId})`,
    );
  };
  ...
  return (
    <BaseTestingPage pageTitle="Add and Remove Markers">
      <div>
        ...
        <IonButton
          id="setOnMarkerClickButton"
          onClick={setOnMarkerClickListener}
        >
          Set On Marker Click / Drag Listeners
        </IonButton>
        <IonButton
          id="removeOnMarkerClickButton"
          onClick={removeOnMarkerClickListener}
        >
